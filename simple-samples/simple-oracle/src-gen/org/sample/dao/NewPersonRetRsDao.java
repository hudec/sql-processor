...
public class NewPersonRetRsDao {
  ...
  public List<Person> newPersonRetRs(final SqlSession sqlSession, final NewPersonRetRs newPersonRetRs, SqlControl sqlControl) {
    ...
    org.sqlproc.engine.SqlProcedureEngine sqlFunNewPersonRetRsDao = sqlEngineFactory.getCheckedProcedureEngine("FUN_NEW_PERSON_RET_RS");
    List<Person> list = sqlFunNewPersonRetRsDao.callQuery(sqlSession, Person.class, newPersonRetRs, sqlControl);
    if (logger.isTraceEnabled()) {
