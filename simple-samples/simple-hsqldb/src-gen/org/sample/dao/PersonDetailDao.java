...
public class PersonDetailDao {
    ...
    public PersonDetail insert(final SqlSession sqlSession, final PersonDetail personDetail, SqlControl sqlControl) {
        ...
        org.sqlproc.engine.SqlCrudEngine sqlInsertPersonDetail = sqlEngineFactory
                .getCheckedCrudEngine("INSERT_PERSON_DETAIL");
    ...
    public PersonDetail get(final SqlSession sqlSession, final PersonDetail personDetail, SqlControl sqlControl) {
        ...
        org.sqlproc.engine.SqlCrudEngine sqlGetEnginePersonDetail = sqlEngineFactory
                .getCheckedCrudEngine("GET_PERSON_DETAIL");
        // sqlControl = getMoreResultClasses(personDetail, sqlControl);
        PersonDetail personDetailGot = sqlGetEnginePersonDetail.get(sqlSession, PersonDetail.class, personDetail,
                sqlControl);
        if (logger.isTraceEnabled()) {
    ...
    public int update(final SqlSession sqlSession, final PersonDetail personDetail, SqlControl sqlControl) {
        ...
        org.sqlproc.engine.SqlCrudEngine sqlUpdateEnginePersonDetail = sqlEngineFactory
                .getCheckedCrudEngine("UPDATE_PERSON_DETAIL");
    ...
    public int delete(final SqlSession sqlSession, final PersonDetail personDetail, SqlControl sqlControl) {
        ...
        org.sqlproc.engine.SqlCrudEngine sqlDeleteEnginePersonDetail = sqlEngineFactory
                .getCheckedCrudEngine("DELETE_PERSON_DETAIL");
    ...
    public List<PersonDetail> list(final SqlSession sqlSession, final PersonDetail personDetail,
        ...
        // sqlControl = getMoreResultClasses(personDetail, sqlControl);
        List<PersonDetail> personDetailList = sqlEnginePersonDetail.query(sqlSession, PersonDetail.class, personDetail,
                sqlControl);
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "sql list personDetail size: " + ((personDetailList != null) ? personDetailList.size() : "null"));
        }
    ...
    public int query(final SqlSession sqlSession, final PersonDetail personDetail, SqlControl sqlControl,
            final SqlRowProcessor<PersonDetail> sqlRowProcessor) {
        if (logger.isTraceEnabled()) {
        ...
        org.sqlproc.engine.SqlQueryEngine sqlEnginePersonDetail = sqlEngineFactory
                .getCheckedQueryEngine("SELECT_PERSON_DETAIL");
        // sqlControl = getMoreResultClasses(personDetail, sqlControl);
        int rownums = sqlEnginePersonDetail.query(sqlSession, PersonDetail.class, personDetail, sqlControl,
                sqlRowProcessor);
        if (logger.isTraceEnabled()) {
    ...
    public int query(final PersonDetail personDetail, SqlControl sqlControl,
            final SqlRowProcessor<PersonDetail> sqlRowProcessor) {
        return query(sqlSessionFactory.getSqlSession(), personDetail, sqlControl, sqlRowProcessor);
    }

    public int query(final SqlSession sqlSession, final PersonDetail personDetail,
            final SqlRowProcessor<PersonDetail> sqlRowProcessor) {
    ...
    public List<PersonDetail> listFromTo(final SqlSession sqlSession, final PersonDetail personDetail,
        ...
        personDetail.setInitAssociations_(new java.util.HashSet<String>());
        final java.util.List<Long> ids_ = sqlEnginePersonDetail.query(sqlSession, Long.class, personDetail, sqlControl);
        personDetail.setInitAssociations_(initAssociations);

        List<PersonDetail> personDetailList = new java.util.ArrayList<PersonDetail>();
        if (!ids_.isEmpty()) {
            org.sqlproc.engine.impl.SqlStandardControl sqlc = new org.sqlproc.engine.impl.SqlStandardControl(
                    sqlControl);
            ...
            final SqlRowProcessor<PersonDetail> sqlRowProcessor = new SqlRowProcessor<PersonDetail>() {
                @Override
                public boolean processRow(PersonDetail result, int rownum)
                        throws org.sqlproc.engine.SqlRuntimeException {
                    map.put(result.getId().getId(), result);
                    return true;
                }
            };
            sqlEnginePersonDetail.query(sqlSession, PersonDetail.class, new PersonDetail()._setIds_(ids_), sqlc,
                    sqlRowProcessor);
            for (Long id : ids_)
