...
ALTER TABLE PERSON ADD (GENDER VARCHAR(1) CHECK (GENDER IN ('M', 'F')));
...
ALTER TABLE CONTACT ADD (TYPE INTEGER CHECK (TYPE IN (0, 1)));
...
CREATE PROCEDURE NEW_PERSON(OUT newid INTEGER, date_of_birth DATE, ssn VARCHAR(20), first_name VARCHAR(100), last_name VARCHAR(100));
  INSERT INTO PERSON (DATE_OF_BIRTH, SSN, FIRST_NAME, LAST_NAME) 
    VALUES (date_of_birth, ssn, first_name, last_name);
  SELECT FIRST 1 dbinfo('bigserial') INTO newid FROM systables;
END PROCEDURE;

CREATE FUNCTION NEW_PERSON_RET_RS(date_of_birth DATE, ssn VARCHAR(20), first_name VARCHAR(100), last_name VARCHAR(100))
    RETURNING INTEGER AS id, DATE AS date_of_birth, VARCHAR (100) AS ssn, 
              VARCHAR(100) AS first_name, VARCHAR(100) AS last_name
  DEFINE IDr INTEGER;
  ...
  SELECT ID, DATE_OF_BIRTH, SSN, FIRST_NAME, LAST_NAME
    INTO IDr, DATE_OF_BIRTHr, SSNr, FIRST_NAMEr, LAST_NAMEr
    FROM PERSON WHERE ID = temp_id;
  RETURN IDr, DATE_OF_BIRTHr, SSNr, FIRST_NAMEr, LAST_NAMEr;
END FUNCTION;
