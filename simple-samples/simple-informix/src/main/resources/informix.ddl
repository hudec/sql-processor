...
CREATE PROCEDURE FUN_LONG(count BIGINT) RETURNING BIGINT;
BEGIN
  DEFINE ind INTEGER;
  DEFINE id BIGINT;
  DEFINE result SET (BIGINT NOT NULL);
  FOR ind = 0 TO count
  	insert into table(result) values (ind);
  END FOR;
  FOREACH select * into id from table(result) 
  	RETURN id with resume;
  END FOREACH;
END;
END PROCEDURE;
