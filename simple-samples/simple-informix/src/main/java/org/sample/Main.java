...
import org.sample.dao.FunLongDao;
...
public class Main {
	...
	private static final String[] DB_CLEAR = new String[] {
			"delete from contact", "delete from person" };
	...
	public Main() throws SQLException {
		...
		funLongDao = new FunLongDao(sqlFactory, sessionFactory);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// insert
		Person jan = main.insertPersonContacts(new Person("Jan", "Jansky",
				PersonGender.MALE), new Contact()._setAddress("Jan address 1")
				._setPhoneNumber("111-222-3333")._setType(ContactType.HOME));
		Person janik = main.insertPersonContacts(new Person("Janik", "Janicek",
				PersonGender.MALE), new Contact()
				._setAddress("Janik address 1")._setType(ContactType.BUSINESS));
		Person honza = main.insertPersonContacts(
				new Person("Honza", "Honzovsky", PersonGender.MALE),
				new Contact()._setAddress("Honza address 1")._setType(
						ContactType.HOME),
				new Contact()._setAddress("Honza address 2")._setType(
						ContactType.BUSINESS));
		Person honzik = main.insertPersonContacts(new Person("Honzik",
				"Honzicek", PersonGender.MALE));
		Person andrej = main.insertPersonContacts(
				new Person("Andrej", "Andrejcek", PersonGender.MALE)
						._setSsn("123456789"),
				new Contact()._setAddress("Andrej address 1")
						._setPhoneNumber("444-555-6666")
		...
		Assert.assertEquals("Andrej address 1", p.getContacts().get(0)
				.getAddress());
		Assert.assertEquals("444-555-6666", p.getContacts().get(0)
				.getPhoneNumber());
		...
		person.setInit(Person.Association.contacts);
		list = main.personDao.list(person,
				new SqlStandardControl().setDescOrder(Person.ORDER_BY_ID));
		Assert.assertEquals(5, list.size());
		Assert.assertEquals("Honzicek", list.get(1).getLastName());
		list = main.personDao
				.list(person, new SqlStandardControl()
						.setAscOrder(Person.ORDER_BY_LAST_NAME));
		Assert.assertEquals(5, list.size());
		Assert.assertEquals("Honzovsky", list.get(2).getLastName());
		person = new Person();
		list = main.personDao.list(person, new SqlStandardControl()
				.setAscOrder(Person.ORDER_BY_LAST_NAME).setMaxResults(2));
		...
		java.sql.Timestamp result = main.anHourBeforeDao
				.anHourBefore(anHourBefore);
