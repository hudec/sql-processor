...
@parser::members {
 ...
 SqlMetaOperator lastOperator = null;
  ...
  boolean addStatement(SqlProcessor processor, String type, String name, String statement, List<String> activeFilters, List<String> filters) {
    List<ErrorMsg> errorsList = getPartialErrors();
    processor.addMetaStatement(type, name, statement, null, errorsList, activeFilters, filters.toArray(new String[0]));
    return errorsList == null;
  }

  boolean addMapping(SqlProcessor processor, String type, String name, String mapping, List<String> activeFilters, List<String> filters) {
    List<ErrorMsg> errorsList = getPartialErrors();
    processor.addMappingRule(type, name, mapping, null, errorsList, activeFilters, filters.toArray(new String[0]));
    return errorsList == null;
  }
...
@init {$processor = new SqlProcessor(defaultFeatures, onlyStatements); boolean skip; List<String> activeFilters;}
        ...
        WS* (
         (name=IDENT {skip=doSkip(onlyStatements,$name.text);} LPAREN type=STATEMENT {activeFilters = new ArrayList<String>();} (COMMA filter=IDENT {activeFilters.add($filter.text);})* RPAREN EQUALS
           optionCtx=option[$name.text] {addStatement($processor, $type.text, $name.text, $optionCtx.text.toString(), activeFilters, filters);} SEMICOLON WS*)
         | (name=IDENT {skip=doSkip(onlyStatements,$name.text);} LPAREN type=MAPPING {activeFilters = new ArrayList<String>();} (COMMA filter=IDENT {activeFilters.add($filter.text);})* RPAREN EQUALS
           optionCtx=option[$name.text] {addMapping($processor, $type.text, $name.text, $optionCtx.text.toString(), activeFilters, filters);} SEMICOLON WS*)
