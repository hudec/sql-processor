...
public class TestMoreInhertance extends TestDatabase {
    ...
    public void testNextInheritance() {
        ...
        assertEquals(Long.valueOf(1), s.getLibrary().getId());
        assertNotNull(s.getBillingDetails());
        assertEquals(2, s.getBillingDetails().size());
        bd = s.getBillingDetails().get(0);
        assertEquals(Long.valueOf(1), bd.getId());
        assertEquals("CC", bd.getType());
        assertTrue(bd instanceof CreditCard);
        cc = (CreditCard) bd;
        assertEquals(Long.valueOf(123456L), cc.getNumber());
        assertEquals(Short.valueOf((short) 11), cc.getExpMonth());
        assertEquals(Short.valueOf((short) 12), cc.getExpYear());
        bd = s.getBillingDetails().get(1);
        assertEquals(Long.valueOf(2), bd.getId());
        ...
        s = list.get(1);
        assertEquals(Long.valueOf(2), s.getId());
        ...
        assertEquals(2, s.getBillingDetails().size());
        bd = s.getBillingDetails().get(0);
        assertEquals(Long.valueOf(3), bd.getId());
        assertEquals("CC", bd.getType());
        assertTrue(bd instanceof CreditCard);
        cc = (CreditCard) bd;
        assertEquals(Long.valueOf(234567), cc.getNumber());
        assertEquals(Short.valueOf((short) 10), cc.getExpMonth());
        assertEquals(Short.valueOf((short) 11), cc.getExpYear());
        bd = s.getBillingDetails().get(1);
        assertEquals(Long.valueOf(4), bd.getId());
        assertEquals("BA", bd.getType());
        assertTrue(bd instanceof BankAccount);
        ba = (BankAccount) bd;
